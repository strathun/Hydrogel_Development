%% experiment_gelImpedance_Day0Cumulative_20211208
% Combines Day 0 gel data from 1123 and 1208.   
...
% Calculate means/std
[~, numMeasures] = size(gamryStructure_EIS_gel_2);
meanArray = [];
PhaseArray = [];
meanMeanArray = [];
stdMeanArray = [];
...
jj = 1;
for ii = 1:numMeasures
    meanArray = [meanArray gamryStructure_EIS_gel_2(ii).Zmag];
    PhaseArray = [PhaseArray gamryStructure_EIS_gel_2(ii).Phase];
    if mod(ii,3) == 0
        meanMeanArray(:,jj) = mean(meanArray, 2);
        stdMeanArray(:,jj) = std(meanArray, 0, 2);
        meanPhaseArray(:,jj) = mean(PhaseArray, 2);
...
for ii = 1:3
    jj = pointerArray(ii);
    yyaxis left
    errorbar( gamryStructure_EIS_gel_2(1).f, ...
            meanMeanArray(:,jj), ...
            stdMeanArray(:,jj))
    hold on
end
...
%% Impedance and phase 11/24 and 12/08 combined averages
% Calculate means/std
% gel
meanArray = [];
PhaseArray = [];
...
for ii = 1:numMeasures
    jj = pointerArray(ii);
    meanArray = [meanArray gamryStructure_EIS_gel(jj).Zmag];
    PhaseArray = [PhaseArray gamryStructure_EIS_gel(jj).Phase];
end
...
for ii = 1:numMeasures
    jj = pointerArray(ii);
    meanArray = [meanArray gamryStructure_EIS_gel_2(jj).Zmag];
    PhaseArray = [PhaseArray gamryStructure_EIS_gel_2(jj).Phase];
end
